...
if __name__ == '__main__':
    ...
    nPs = len(pthr)
    mkdir(args.dir_name)
    all_prob = np.array([[0.05, 0.05, 0.05, 0.05, 0.8],
    ...
    # np.random.seed(0)
    for meth in methLis:
        mkdir(os.path.join(args.dir_name, meth[0]+'_'+meth[1]))
                ...
                for l in range(nLoops):
                    ...
                    for meth in methLis:
                        ...
                        for fnm_idx, fnm in enumerate(outNm):
                            with open(os.path.join(args.dir_name, meth[0]+'_'+meth[1], 'TP_'+fnm+'_share'+str(allocShare[-2:])+'.csv'), 'a+') as f_TP:
                                np.savetxt(f_TP, TParr[fnm_idx].reshape(1, -1), fmt='%d', delimiter=',')
                            with open(os.path.join(args.dir_name, meth[0]+'_'+meth[1], 'FP_'+fnm+'_share'+str(allocShare[-2:])+'.csv'), 'a+') as f_FP:
                                np.savetxt(f_FP, FParr[fnm_idx].reshape(1, -1), fmt='%d', delimiter=',')
                            with open(os.path.join(args.dir_name, meth[0]+'_'+meth[1], 'FN_'+fnm+'_share'+str(allocShare[-2:])+'.csv'), 'a+') as f_FN:
