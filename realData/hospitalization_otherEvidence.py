...
class cenDecen:
    ...
    def get_combinePval(self, change_share):
        if change_share: 
            ...
            for i in range(self.TSlen - self.l_shift):
                testpval = np.array(self.pval_subTS)[:, i]
                combinedP[i] = combinePval(testpval, combMethod = self.combMethod, weightMethod = self.weightMethod, shares = self.allocProp)
...
if __name__ == '__main__':
    ...
    methLis = [['Stouffer', 'default'], ['Fisher', 'default'], ['wFisher', 'default'], ['Pearson', 'default'], ['LargestSite', 'default']]
    dir_name = 'rewrite_lag_hospitalization'
    allocLis = ['total_adult_patients_hospitalized_confirmed_covid_7_day_sum', 'total_beds_7_day_avg', 'inpatient_beds_used_7_day_sum', 'inpatient_beds_7_day_sum', 'total_icu_beds_7_day_sum', 'previous_day_total_ED_visits_7_day_sum']
    for sel_key in allocLis:
        df_out[sel_key] = [eval(df_out[sel_key][i]) for i in range(df_out.shape[0])]
    np.random.seed(0)
    mkdir(dir_name)
    for loc_idx in range(len(locSelLis)):
        ...
        for allocMeth in allocLis:
            originalData = cenDecen(date_value, subTS, allocProp = np.array(df_out[allocMeth][loc_idx]), growthRateThr = growthRateThr, weightMethod = 'unweighted', pval_thr = pval_thr_ref, window4Pval = window4Pval, updating_period = None, training_period = None, share_lag = None, rep_per_wk = rep_per_wk)
            ...
            if len(ref_alarm) > 0:
                for m_idx, meth in enumerate(methLis):
                    ...
                    originalData.get_combinePval(change_share = False)
                    ...
                    with open(os.path.join(dir_name, allocMeth+'_'+meth[0]+'_'+meth[1]+'_recall.csv'), 'a+') as f_TP:
                        np.savetxt(f_TP, recall.reshape(1, -1), fmt='%.5f', delimiter=',')
                    with open(os.path.join(dir_name, allocMeth+'_'+meth[0]+'_'+meth[1]+'_precision.csv'), 'a+') as f_FP:
